...
use crate::{
    dataset::SpectraData,
    error::SpectraError,
    experiment_config::{ExperimentConfig, standard::StandardConfig},
    holdout::Holdout,
...
pub fn run() -> Result<(), SpectraError> {
    ...
    let experiment_dir = format!("./experiments/experiment{}", experiment_config.experiment_num());
    ...
    for holdout in holdouts {
        ...
        let artifact_dir = format!("{}/holdout_{}", experiment_dir, holdout.holdout_number(),);
        ...
        let results_path = PathBuf::from(results_dir).join(format!(
            "experiment{}_holdout_{}_results.csv", experiment_config.experiment_num(),
            holdout.holdout_number(),
        ));
